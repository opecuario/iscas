...
function NovaPage() {
    ...
    async function carregar() {
      if (idParam) {
        ...
        if (s && s.tipo === "recria_engorda") {
          ...
          setVariante(etapa);
        } else if (s && s.tipo === "cria") {
          router.replace(`/nova/cria?id=${idParam}`);
          return;
        } else {
          router.replace("/nova");
        }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      if (!anterior || anterior.tipo !== "recria_engorda") return;
  ...
  async function salvar(finalizar: boolean) {
    ...
    const anterior =
      anteriorRaw && anteriorRaw.tipo === "recria_engorda" ? anteriorRaw : null;
    ...
    if (etapaNova === "finalizado") {
      ...
      router.replace(`/nova/recria-engorda?id=${novoId}&etapa=${etapaNova}`);
